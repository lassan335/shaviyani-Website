...
## The deployment topology — there is no production yet
This is **Shaviyani Pro**, a Next.js 14 + Prisma/SQLite app, currently **local-only**: no hosting
platform, no managed database, no git remote, no CI. Update this section for real the moment any of
these get set up — until then, treat every run as a **local preflight only**, never a real deploy.

- **Local dev DB vs Production DB.** Same file today: `prisma/dev.db` (SQLite, gitignored, path from
  `DATABASE_URL` in `.env`). There is no separate prod DB. When a real DB is chosen (the schema's `cuid()`
  ids and portable Prisma types are written to make a Postgres move low-friction — e.g. Supabase/Railway
  as the original plan proposed), update this line and stop treating local and prod as the same store.
- **What the host deploys from.** No host configured. When one is (e.g. Vercel), it will deploy from a
  git branch, not the local working tree — reconcile local ↔ remote ↔ live once that exists.
- **Does the build run migrations?** `npm run build` runs `prisma generate && next build` — it does
  **not** run `prisma db push` or a real migration. Schema sync today is a manual `npx prisma db push`
  against the local SQLite file. Once on Postgres, switch to `prisma migrate deploy` with committed
  migration files (`prisma migrate dev` to generate them) — `db push` is a dev-only, no-history tool and
  should stop being used against any shared/prod database.
- **Single-process DB locks.** SQLite is file-based and can lock under concurrent writers. If a local
